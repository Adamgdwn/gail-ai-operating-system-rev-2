...
> **Historical master pathway and active routing index.** This file keeps the
> long-running build ledger and stable pathway history for this project.
> Current execution may be routed to a compact dated work packet named by
> `START_HERE.md`. All prior pathway, deployment-plan, and build-plan documents
> in this repo must carry `Status: superseded` and reference this file unless a
> newer active packet explicitly supersedes a bounded section.
...
Inputs: Graphify acceleration readiness plan, active pathway, engineering
standards, project-control classification, architecture key decisions, risk
register, and canonical Graphify governance. Outputs: plan-local GA-A through
GA-E work packets inside
`docs/decisions/2026-06-27 - Graphify Acceleration Readiness Plan.md`, plus
active pathway and changelog updates. Acceptance: each phase has named chunks
with objective, inputs, outputs, acceptance, validation, and stop boundaries;
the sequence keeps contract validation before export preview, publication
before adapter design, and explicit owner approval before live Graphify ingest;
Chunk Twenty remains the default implementation path unless Adam later promotes
a GA-B slice. Validation: governance preflight, repo-local Graphify query,
document review, diff check, Graphify incremental update, commit, push. Stop:
...
## Pre-Chunk Twenty - Execute Graphify Acceleration Phase B
...
`contains_raw_payload: false`; GA-B3 adds classification helpers for safe
summaries, refs, typed edge labels, raw-payload hints, generated graph output,
log/audio dumps, client payload markers, live connector markers, and secret
markers without echoing unsafe contents; GA-B4 adds pure local record builders
for `Action`, `AuthorityEnvelope`, and `EvidencePacket`; GA-B5 adds
deterministic fingerprints that exclude only `generated_at` and `fingerprint`;
GA-B6 exposes the stable local contract through the package root. Validation:
focused Graphify acceleration unit tests, full Python test suite, governance
preflight, diff check, Graphify incremental update, commit, push. Stop: before
persistence, preview files, Graphify calls, HTTP APIs, cloud placement,
adapter work, TypeScript schema publication, live connectors, live data,
runtime hooks, or execution authority.

## Pre-Chunk Twenty - Decide Graphify Preview Retention
...
Inputs: GA-B contract and emitters, current `.gitignore`, relay store behavior,
evidence packet direction, Graphify acceleration readiness plan, document
control standard, and retention/security boundaries. Outputs:
`docs/decisions/2026-06-27 - Graphify Preview Retention Decision.md`,
Graphify acceleration plan update, active pathway update, source/context
routing update, and changelog note. Acceptance: default recommendation remains
ignored local preview output unless Adam approves controlled retention; cleanup
expectations and commit boundaries are named; no Graphify ingest path is
implied by the preview location; no preview writes are added. Validation:
governance preflight, targeted document/source review, document review, and
diff check. Stop: before preview writer creation, preview output, persistent
export store, relay-store-adjacent retention, Graphify calls, adapters, HTTP
...
7. Open M365 dry-run, API router, evidence, and runtime proof files only when
   Adam gives the go-ahead to execute CMS-B or to repair a later CI issue.
...
As of 2026-06-27T17:03:09-06:00, Adam's promoted local-only Graphify
acceleration Phase B is integration complete. Rev 2 now has a local
`GraphifyAccelerationRecord` contract, sanitizer classification helpers,
pure local Action/AuthorityEnvelope/EvidencePacket graph-fact builders,
deterministic fingerprints, and package-root exports. No persistence, preview
output, schema artifact, Graphify adapter, transport, HTTP API, cloud
placement, live connector, runtime hook, or execution authority has been
approved or created.
As of 2026-06-27T17:15:09-06:00, GA-C1 is draft complete at
`docs/decisions/2026-06-27 - Graphify Preview Retention Decision.md`. Future
local Graphify acceleration previews should default to ignored developer
artifact output under `tmp/graphify-acceleration-preview/`. Generated preview
